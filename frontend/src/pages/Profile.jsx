...
function Profile() {
  ...
  useEffect(() => {
    if (user && currentUser && userId !== currentUser._id) {
      const followerIds = user.followers?.map(f => typeof f === 'string' ? f : f._id);
  ...
  const handleAvatarClick = () => {
    ...
    if (confirmUpload) fileInputRef.current.click();
  };
  ...
  if (!user || !followStatusLoaded) return <div className="text-center p-6">Loading profile...</div>;
